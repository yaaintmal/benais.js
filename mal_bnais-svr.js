// server_plain.js  <-- replace your existing server file or keep this as a new one
...
async function getOllamaResponse(prompt) {
  const body = { model: process.env.LLM_MODEL || 'gemma3', messages: [{ role: 'user', content: prompt }], stream: false };
